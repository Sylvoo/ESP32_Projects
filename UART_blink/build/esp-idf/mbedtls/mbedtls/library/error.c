...
/* Begin Auto-Generated Code. */
  ...
  case -(MBEDTLS_ERR_PKCS7_FEATURE_UNAVAILABLE):
    return (
        "PKCS7 - Unavailable feature, e.g. anything other than signed data");
  case -(MBEDTLS_ERR_PKCS7_INVALID_VERSION):
    return (
        "PKCS7 - The PKCS #7 version element is invalid or cannot be parsed");
  ...
  case -(MBEDTLS_ERR_PKCS7_INVALID_ALG):
    return (
        "PKCS7 - The algorithm tag or value is invalid or cannot be parsed");
  case -(MBEDTLS_ERR_PKCS7_INVALID_CERT):
    return (
        "PKCS7 - The certificate tag or value is invalid or cannot be parsed");
  ...
  case -(MBEDTLS_ERR_SSL_UNSUPPORTED_EXTENSION):
    return ("SSL - Client received an extended server hello containing an "
  ...
  case -(MBEDTLS_ERR_SSL_RECEIVED_NEW_SESSION_TICKET):
    return (
        "SSL - * Received NewSessionTicket Post Handshake Message. This error "
        "code is experimental and may be changed or removed without notice");
  ...
  case -(MBEDTLS_ERR_SSL_RECEIVED_EARLY_DATA):
    return (
        "SSL - * Early data has been received as part of an on-going "
        "handshake. This error code can be returned only on server side if and "
        "only if early data has been enabled by means of the "
        "mbedtls_ssl_conf_early_data() API. This error code can then be "
        "returned by mbedtls_ssl_handshake(), mbedtls_ssl_handshake_step(), "
        "mbedtls_ssl_read() or mbedtls_ssl_write() if early data has been "
        "received as part of the handshake sequence they triggered. To read "
        "the early data, call mbedtls_ssl_read_early_data()");
  ...
  case -(MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER):
    return ("SSL - A field in a message was incorrect or inconsistent with "
            "other fields");
