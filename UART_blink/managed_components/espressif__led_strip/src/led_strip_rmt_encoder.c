...
esp_err_t rmt_new_led_strip_encoder(const led_strip_encoder_config_t *config,
  ...
  ESP_GOTO_ON_FALSE(config && ret_encoder, ESP_ERR_INVALID_ARG, err, TAG,
                    "invalid argument");
  ESP_GOTO_ON_FALSE(config->led_model < LED_MODEL_INVALID, ESP_ERR_INVALID_ARG,
                    err, TAG, "invalid led model");
  ...
  if (config->led_model == LED_MODEL_SK6812) {
    bytes_encoder_config = (rmt_bytes_encoder_config_t){
        ...
        .flags.msb_first =
            1 // SK6812 transfer bit order: G7...G0R7...R0B7...B0(W7...W0)
    };
  ...
  uint32_t reset_ticks =
      config->resolution / 1000000 * 280 /
      2; // reset code duration defaults to 280us to accomodate WS2812B-V5
