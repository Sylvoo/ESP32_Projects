...
static esp_err_t led_strip_rmt_set_pixel(led_strip_t *strip, uint32_t index,
  ...
  // In thr order of GRB, as LED strip like WS2812 sends out pixels in this
  // order
...
static esp_err_t led_strip_rmt_refresh(led_strip_t *strip) {
  ...
  ESP_RETURN_ON_ERROR(rmt_enable(rmt_strip->rmt_chan), TAG,
                      "enable RMT channel failed");
  ESP_RETURN_ON_ERROR(
      rmt_transmit(rmt_strip->rmt_chan, rmt_strip->strip_encoder,
                   rmt_strip->pixel_buf,
                   rmt_strip->strip_len * rmt_strip->bytes_per_pixel, &tx_conf),
      TAG, "transmit pixels by RMT failed");
  ESP_RETURN_ON_ERROR(rmt_tx_wait_all_done(rmt_strip->rmt_chan, -1), TAG,
                      "flush RMT channel failed");
  ESP_RETURN_ON_ERROR(rmt_disable(rmt_strip->rmt_chan), TAG,
