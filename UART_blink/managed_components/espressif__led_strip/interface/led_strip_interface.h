...
struct led_strip_t {
  ...
  /**
   * @brief Set RGBW for a specific pixel. Similar to `set_pixel` but also set
   * the white component
   *
   * @param strip: LED strip
   * @param index: index of pixel to set
   ...
   * error occurred
   */
  esp_err_t (*set_pixel_rgbw)(led_strip_t *strip, uint32_t index, uint32_t red,
                              uint32_t green, uint32_t blue, uint32_t white);
